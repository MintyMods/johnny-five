...
var Controllers = {
  ...
  // Previously GROVE_I2C_MOTOR_DRIVER
  GROVE_I2C_L298: {
    REGISTER: {
      value: {
        ADDRESS: 0x0F,
    ...
    initialize: {
      value: function(opts) {
        var state = priv.get(this);
        var shared = priv.get("GROVE_I2C_L298");
        ...
        if (!shared) {
          ...
          priv.set("GROVE_I2C_L298", shared);
        }
  ...
  GROVE_I2C_TB6612FNG: {
    ...
    COMMANDS: {
      value: {
        ...
        CCW: 0x03,
        STANDBY: 0x04, // @todo
        NOT_STANDBY: 0x05, // @todo
        STEPPER_RUN: 0x06, // @todo
        STEPPER_STOP: 0x07, // @todo
        STEPPER_KEEP_RUN: 0x08, // @todo
        SET_ADDR: 0x11,
      },
    },
    MOTOR: {
    ...
    initialize: {
      value: function(opts) {
        var state = priv.get(this);
        var shared = priv.get("GROVE_I2C_TB6612FNG");
        ...
        if (!shared) {
          ...
          priv.set("GROVE_I2C_TB6612FNG", shared);
        }
    ...
    setPWM: {
      value: function(pin, value) {
        var state = priv.get(this);
        var direction = state.shared.direction[state.pin]
        var speed = Board.constrain(value, 0, 255) | 0;
        state.shared.speed[state.pin] = speed;
        var motor = state.pin == "A" ? this.MOTOR.A : this.MOTOR.B;
        this.io.i2cWrite(this.address, direction, [motor, speed]);
      }
    },
    setPin: {
      value: function(pin, value) {
        var state = priv.get(this);
        var speed = state.shared.speed[state.pin];
        var direction = value ? this.COMMANDS.CW : this.COMMANDS.CCW;
        state.shared.direction[state.pin] = direction;
        var motor = state.pin == "A" ? this.MOTOR.A : this.MOTOR.B;
        this.io.i2cWrite(this.address, direction, [motor, speed]);
      }
    },
...
// Deprecate GROVE_I2C_MOTOR_DRIVER for additional Grove boards
Controllers.GROVE_I2C_MOTOR_DRIVER = Controllers.GROVE_I2C_L298;
